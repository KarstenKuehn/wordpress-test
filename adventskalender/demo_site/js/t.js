...
et_addEvent(
    ...
    },
    {
        "name": "pdf",
        "type": "download",
        "pattern": "^(https?:\\\/\\\/)?(www\\.)?(.*\\.)?(lotto-bayern\\.de).+\\.pdf$"
    }
];
...
var et_deliveryHash = "8gfgP3x2X/stMBPHdWwAaefrv1N0WZ3M";

var cc_autoPageNameRegistration = 'title';
var cc_getParamsWhiteList = [];
ET_Event = new etEvent("eKxNd9", et_server);
...
var cc_deltaTime 	= 16074353591720-(new Date().getTime()*10);
...
'use strict';var SAMPLING_RATE=0.05;var shouldScrollDepthBeActive=false;_etracker.addEvent(function(){if(_etracker.isTrackingEnabled()){if(_isCookieSet()){shouldScrollDepthBeActive=_isCookieTrue();}else{shouldScrollDepthBeActive=Math.random()<=SAMPLING_RATE;_setCookie(shouldScrollDepthBeActive);}}if(shouldScrollDepthBeActive&&!window.et_ScrollDepthUtils.isInIframe()){var scrollDepth=new ScrollDepthTracker(new Date().getTime());scrollDepth.sendStoredBuckets();_etracker.addOnLoad(scrollDepth.startTracking);}try{localStorage.removeItem('et_scroll_depth');}catch(e){}});function _isCookieSet(){return !!document.cookie.match(/isSdEnabled=/);}function _isCookieTrue(){return !!document.cookie.match(/isSdEnabled=true/);}function _setCookie(isTrue){et_setCookieValue('isSdEnabled',isTrue,1,undefined,false);}
...
})();function _etc_start()
{
	var c = "";
		...
		if(
			...
			window.et_hasOptInPushDomain = true;
			window.et_optInPushDomain = "lotto-bayern." + et_optInPushTopLevelDomain;
			window.et_optInNative = false;

			(function(){
				var head = document.getElementsByTagName("head")[0] || document.documentElement;
				var script = document.createElement("script");
				script.src = _etracker.getConfigValue("signalizeCodeUrl") || 'https://api.signalize.com/accounts/XeKxNd9/signalize.min.js';
	...
	etCommerce.etCommerceLoad('eKxNd9');
	etCommerce.doPreparedEvents();        et_setupPageExitBeacon('eKxNd9');
