...
function get_custom_logo_bs( $blog_id = 0 ) {
	$html          = '';
	$switched_blog = false;
	...
	if ( $custom_logo_id ) {
		...
		if ( $unlink_homepage_logo && is_front_page() && ! is_paged() ) {
			...
			$html = sprintf(
				'<a href="%1$s" class="custom-logo-link" rel="home"%2$s aria-label="zurück zur Startseite">%3$s</a>',
				esc_url( home_url( '/' ) ),
				$aria_current,
				$image
			);
 ...
 */
function bs_site_logo( $args = array(), $echo = true ) {
	$logo       = get_custom_logo_bs();
...
//		'single_wrap' => '<div class="%1$s faux-heading">%2$s</div>',
	 ...
	 */
	$html = apply_filters( 'bs_site_logo', $html, $args, $classname, $contents );
...
function bs_site_logo_url( $args = array(), $echo = true ) {
